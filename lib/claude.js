...
export async function generateMealPlan({ protein, carbs, fat, calories, liked, disliked, likedMeals = [], weeklyContext = {} }) {
  ...
  const contextLines = [
    ...
    weeklyContext.selectedTemplates?.length && `Required recipes to include (use these exact names and macros, scaled by the given quantity): ${weeklyContext.selectedTemplates.map(t => `"${t.name}" for ${t.meal} ×${t.quantity} = ${Math.round(t.calories * t.quantity)} cal | ${Math.round(t.protein * t.quantity)}g P | ${Math.round(t.carbs * t.quantity)}g C | ${Math.round(t.fat * t.quantity)}g F`).join('; ')}.`,
  ].filter(Boolean).join(' ')
