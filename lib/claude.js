...
- followUp (string — if needsQuantity is true, ask: "Did you follow this recipe exactly as planned? Just reply YES, or give me the amounts (e.g. 150g chicken, 1 cup rice)."; otherwise empty string "")
...
export async function generateMealPlan({ protein, carbs, fat, calories, liked, disliked, likedMeals = [], weeklyContext = {} }) {
  ...
  const contextLines = [
    ...
    weeklyContext.recipeIdeas && `Recipe inspiration: the user wants to try — "${weeklyContext.recipeIdeas}". Build at least one meal this week inspired by this idea, fitting the macro targets. Create a full recipe with ingredients and macros for it.`,
  ].filter(Boolean).join(' ')

  const DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
  const requiredMealsSection = weeklyContext.selectedTemplates?.length
    ? (() => {
        // Assign each required meal to a specific day, cycling through Mon–Sun
        const assignments = weeklyContext.selectedTemplates.map((t, i) => ({
          ...t,
          day: DAYS[i % 7],
          displayName: t.quantity > 1 ? `${t.name} ×${t.quantity}` : t.name,
          scaledCal: Math.round(t.calories * t.quantity),
          scaledP: Math.round(t.protein * t.quantity),
          scaledC: Math.round(t.carbs * t.quantity),
          scaledF: Math.round(t.fat * t.quantity),
        }))
        return `
REQUIRED MEAL PLACEMENTS — place each of these on the exact day and meal type shown. Do NOT generate ingredients or bullet points for them — output only the meal type label, the name, and the Macros line. Adjust other meals on that day to keep the daily total near ${calories} cal.
${assignments.map(a => `  • ${a.day} ${a.meal}: ${a.displayName}\n    Macros: ${a.scaledCal} cal | ${a.scaledP}g P | ${a.scaledC}g C | ${a.scaledF}g F`).join('\n')}
`
      })()
    : ''
...
DAILY TARGETS: ${calories} cal | ${protein}g protein | ${carbs}g carbs | ${fat}g fat. Hit these every single day.

Per-meal targets — size each meal to match these before writing its Macros line (within 30 cal / 5g macro):
- Breakfast: ~${Math.round(calories * 0.22)} cal | ~${Math.round(protein * 0.22)}g P | ~${Math.round(carbs * 0.22)}g C | ~${Math.round(fat * 0.22)}g F
- Lunch:     ~${Math.round(calories * 0.26)} cal | ~${Math.round(protein * 0.26)}g P | ~${Math.round(carbs * 0.26)}g C | ~${Math.round(fat * 0.26)}g F
- Dinner:    ~${Math.round(calories * 0.30)} cal | ~${Math.round(protein * 0.30)}g P | ~${Math.round(carbs * 0.30)}g C | ~${Math.round(fat * 0.30)}g F
- Snack:     ~${Math.round(calories * 0.22)} cal | ~${Math.round(protein * 0.22)}g P | ~${Math.round(carbs * 0.22)}g C | ~${Math.round(fat * 0.22)}g F
Total:        ${calories} cal |  ${protein}g P |  ${carbs}g C |  ${fat}g F

Adjust ingredient weights and serving sizes until each meal's Macros line matches its row above.
...
ADDITIONAL RULES:
- Lunch meats and sliced cheese: always use Boar's Head brand (e.g. "Boar's Head Oven Gold Turkey Breast", "Boar's Head Yellow American Cheese"). Use standard nutrition values for Boar's Head products.
- Vegetables: always use fresh vegetables — never frozen. Use fresh broccoli, fresh spinach, fresh bell peppers, fresh zucchini, etc. No frozen vegetable bags.
- Sandwiches: cap at 2 slices of bread maximum. Never use more than 2 slices per sandwich.
...
${requiredMealsSection}Format each day exactly like this:
...
- 1 tbsp honey
Macros: 374 cal | 22g P | 52g C | 4g F

Lunch: Chicken Rice Bowl
- 175g Kirkwood Boneless Skinless Chicken Breast — 40g protein
- 1 cup cooked Season's Choice Steamable Brown Rice (195g)
- 1 cup fresh broccoli florets
- 1 tbsp olive oil
Macros: 442 cal | 46g P | 47g C | 10g F

Dinner: Ground Beef Stir Fry
- 170g Ground Beef 93% Lean (ALDI) — 35g protein
- 1 cup fresh bell pepper and zucchini, sliced
- 2.5 oz dry Reggano Whole Wheat Penne (cooked)
- 2 tbsp low sodium soy sauce
Macros: 510 cal | 42g P | 53g C | 14g F

Snack: Protein Shake
- 1 scoop (37g) Millville Elevation Vanilla Whey Protein Powder — 30g protein
- 1 medium banana
- 1 cup (240ml) Friendly Farms 2% Ultra-Filtered Milk
Macros: 374 cal | 45g P | 42g C | 7g F
