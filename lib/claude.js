...
DAILY CALORIE TARGET: ${calories} calories. Hit this every single day.

Per-meal calorie targets to hit exactly ${calories} cal/day:
- Breakfast: ~${Math.round(calories * 0.22)} cal
- Lunch: ~${Math.round(calories * 0.26)} cal
- Dinner: ~${Math.round(calories * 0.30)} cal
- Snack: ~${Math.round(calories * 0.22)} cal
Total: ${calories} cal

Size every meal to match its target above. If a meal comes in short or over, adjust ingredient weights before writing the Macros line. Do not move on to the next meal until the Macros line matches the target (within 30 cal).
