...
export async function generateMealPlan({ protein, carbs, fat, calories, liked, disliked, likedMeals = [], weeklyContext = {} }) {
  ...
  const requiredMealsSection = weeklyContext.selectedTemplates?.length
    ? `
REQUIRED MEALS — you MUST include every meal listed below somewhere in the 7-day plan. These are pre-saved recipes the user wants in their week. Distribute them across different days.
For each required meal: write ONLY the meal type label, the exact name, and the Macros line. Do NOT add any ingredients or bullet points for these meals.
${weeklyContext.selectedTemplates.map(t => `  • ${t.meal}: ${t.quantity > 1 ? `${t.name} ×${t.quantity}` : t.name} — Macros: ${Math.round(t.calories * t.quantity)} cal | ${Math.round(t.protein * t.quantity)}g P | ${Math.round(t.carbs * t.quantity)}g C | ${Math.round(t.fat * t.quantity)}g F`).join('\n')}
`
    : ''
