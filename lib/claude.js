...
export async function generateMealPlan({ protein, carbs, fat, calories, liked, disliked, likedMeals = [], weeklyContext = {} }) {
  ...
  const contextLines = [
    ...
    weeklyContext.selectedTemplates?.length && `Required pre-saved recipes — CRITICAL: for each of these meals, output ONLY the recipe name and the Macros line. Do NOT generate any ingredients, bullet points, or cooking instructions for them. Just write the meal type label, the name (include ×N in the name if quantity > 1, e.g. "Egg Cups ×3"), then the Macros line and nothing else in between. These are pre-made meals the user already knows how to prepare: ${weeklyContext.selectedTemplates.map(t => `${t.meal}: "${t.quantity > 1 ? `${t.name} ×${t.quantity}` : t.name}" → Macros: ${Math.round(t.calories * t.quantity)} cal | ${Math.round(t.protein * t.quantity)}g P | ${Math.round(t.carbs * t.quantity)}g C | ${Math.round(t.fat * t.quantity)}g F`).join('; ')}.`,
  ].filter(Boolean).join(' ')
