...
async function getPlannerStateBlock() {
  const page = await notion.blocks.children.list({
    block_id: process.env.NOTION_USER_SETTINGS_PAGE_ID,
  })
  return page.results.find(
    (b) => b.type === 'paragraph' && b.paragraph.rich_text?.[0]?.plain_text?.startsWith('__MEAL_PLANNER_STATE__:')
  )
}

export async function getMealPlannerState(chatId) {
  try {
    const block = await getPlannerStateBlock()
    if (!block) return null
    const raw = block.paragraph.rich_text[0].plain_text.replace('__MEAL_PLANNER_STATE__:', '')
    const state = JSON.parse(raw)
    if (String(state.chatId) !== String(chatId)) return null
    return state
  } catch {
...
export async function setMealPlannerState(chatId, questionIndex, responses) {
  const payload = JSON.stringify({ chatId: String(chatId), questionIndex, responses })
  const text = `__MEAL_PLANNER_STATE__:${payload}`

  const existing = await getPlannerStateBlock()
  if (existing) {
    await notion.blocks.update({
      block_id: existing.id,
      paragraph: { rich_text: [{ type: 'text', text: { content: text } }] },
...
export async function clearMealPlannerState(chatId) {
  const block = await getPlannerStateBlock()
  if (!block) return
  await notion.blocks.update({
    block_id: block.id,
