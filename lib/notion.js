...
export async function deleteFoodLog(pageId) {
  try {
    // Try archiving via pages API first
    return await notion.pages.update({ page_id: pageId, archived: true })
  } catch (err) {
    // If already archived, treat as success
    if (err.message?.includes('archived')) return { ok: true }
    // Fall back to blocks.delete
    try {
      return await notion.blocks.delete({ block_id: pageId })
    } catch (err2) {
      if (err2.message?.includes('archived')) return { ok: true }
      throw err2
    }
  }
}
...
export async function deleteMealTemplate(pageId) {
  return notion.blocks.delete({ block_id: pageId })
}
